 ...
 */
static double
get_theta(struct point* reg, int reg_size, double x, double y, image_double modgrad, double reg_angle, double prec) {
...
static int refine(struct point* reg,
    ...
    tau =
        2.0 * sqrt((s_sum - 2.0 * mean_angle * sum) / (double)n + mean_angle * mean_angle); /* 2 * standard deviation */
...
double* LineSegmentDetection(int* n_out,
    ...
    */
    ...
    int minimum_length_square = minLen * minLen;
    ...
    for (; list_p != NULL; list_p = list_p->next)
           ...
           because we are only interested in the exact NOTDEF value */
        {
            ...
            // ADDED Code to filter length
            int x_diff = rec.x1 - rec.x2;
            int y_diff = rec.y1 - rec.y2;
            int square_length = x_diff * x_diff + y_diff * y_diff;
            if (square_length < minimum_length_square)
                continue;
            // (end) ADDED Code to filter length
...
double* lsd_scale_region(int* n_out, double* img, int X, int Y, double scale, int** reg_img, int* reg_x, int* reg_y) {
    ...
    return LineSegmentDetection(n_out,
                                img,
                                X,
                                Y,
                                scale,

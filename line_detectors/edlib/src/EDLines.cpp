...
Mat EDLines::getLineImage()
{
	...
	cvtColor(lineImage, lineImage, COLOR_GRAY2BGR);
	RNG rng( 0xFFFFFFFF );
	for (int i = 0; i < linesNo; i++) {
		line(lineImage, linePoints[i].start, linePoints[i].end, Scalar(rng.uniform(0,255), rng.uniform(0,255), rng.uniform(0,255)), 1, LINE_AA, 0);
	}
...
Mat EDLines::drawOnImage()
{
	...
	RNG rng( 0xFFFFFFFF );
	for (int i = 0; i < linesNo; i++) {
		line(colorImage, linePoints[i].start, linePoints[i].end, Scalar(rng.uniform(0,255), rng.uniform(0,255), rng.uniform(0,255)), 1, LINE_AA, 0);
	}

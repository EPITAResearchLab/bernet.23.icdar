...
public:
	...
	void MetaLineDetection(cv::Mat originalImage,float gausSigma, int gausHalfSize, float gradNoise, std::vector<std::vector<float> > &lines);

private:
	void getInformations(cv::Mat &originalImage,float gausSigma, int gausHalfSize,float p, float gradNoise);

...
bool MetaLine::leastSquareFitting(cluster_t& cluster, float* parameters, float sigma) {
    ...
    if (cluster.pixels[0].x == cluster.pixels[N - 1].x)
        ...
        kCoarse =
            float(cluster.pixels[0].y - cluster.pixels[N - 1].y) / (cluster.pixels[0].x - cluster.pixels[N - 1].x);
        ...
        for (size_t i = 0; i != 8; ++i) {
            ...
            if ((0 <= x) && (x < cols)) {
                ...
                if ((0 <= y) && (y < rows)) {
                    ...
                    if (ptrMI[y * cols + x] < 0 && ptrMI[y * cols + x] != ID)
                        return -1; //-1 for stop merging
    ...
    if (abs(deltaX) > abs(deltaY)) {
        ...
        for (i = xMin + 1; i < xMax; ++i) {
            ...
            if (0 < yInt && yInt < rows_1) {
                ...
                if ((e0 < 0 && e0 != ID) || (e1 < 0 && e1 != ID) || (e2 < 0 && e2 != ID))
                    return false; // find another line
                if (e0 == 1 || e1 == 1 || e2 == 1) {
                    float angleEdge = 0;
                    int Rnt = crossSearch(Point2i(xInt, yInt),
                                          angleEdge,
                                          ID); // 1for finding a edge chain, 0 for non, -1 for another line segment
                    if (Rnt) {
    ...
    while (1) {
        ...
        if (0 < xInt && xInt < cols_1 && 0 < yInt && yInt < rows_1) {
            ...
            if (lineHyps.size()) {
                ...
                if ((double)gap / edge >= 0.25 || edge == 0)
                    break;
                if (gap == 2) {
                    edge = 0;
                    gap = 0;
...
void MetaLine::getInformations(cv::Mat& originalImage, float gausSigma, int gausHalfSize, float p, float gradNoise) {
    int i, j, m, n;
    int grayLevels = 255;
    int apertureSize = 3;
    float anglePer = CV_PI / 8.0;
    float gNoise = gradNoise; // 1.3333
...
bool MetaLine::smartRouting(clusters_list_t& segments, float minDeviation, int minSize) {
    ...
    // sprintf(text,"%d", i);
    // Point
    // mid=times*cv::Point(int(segments[i].pixels[segments[i].size/2].x),int(segments[i].pixels[segments[i].size/2].y));
...
void MetaLine::MetaLineDetection(cv::Mat originalImage,
                                 float gausSigma,
                                 int gausHalfSize,
                                 float gradNoise,
    ...
    getInformations(originalImage, gausSigma, gausHalfSize, p, gradNoise);
...
int MetaLine::lineMerging(int IDCur,
                          line_t& metaLineCur,
                          size_list_t& lineHyps,
                          lines_list_t& metaLines,
                          float thAngle) {
    ...
    if (metaLineCur.points[0].x == metaLineCur.points[numCur - 1].x) {
        ...
        angleCur = atan(float(metaLineCur.points[0].y - metaLineCur.points[numCur - 1].y) /
                        float(metaLineCur.points[0].x - metaLineCur.points[numCur - 1].x));
    ...
    for (i = 0; i < lineHyps.size(); ++i) {
        ...
        if (metaLines[lineHyps[i]].points[0].x == metaLines[lineHyps[i]].points[numTemp - 1].x) {
            ...
            angleTemp = atan(float(metaLines[lineHyps[i]].points[0].y - metaLines[lineHyps[i]].points[numTemp - 1].y) /
                             float(metaLines[lineHyps[i]].points[0].x - metaLines[lineHyps[i]].points[numTemp - 1].x));
    ...
    if (angleMin > thAngle)
        ...
        if (k > 1) {
            ...
            if (dis_s < dis_e && dis_s < thDis) {
                if ((metaLines[metaIDHyp].points[numHyp - 1].y - metaLineCur.points[numCur - 1].y) *
                        (metaLineCur.points[0].y - metaLineCur.points[numCur - 1].y) <
                    0) {
                    // merge
                    for (i = 0; i < numHyp; ++i) {
                        pixel_t& p = metaLineCur.points.push_back();
            ...
            if (dis_e < dis_s && dis_e < thDis) {
                if ((metaLines[metaIDHyp].points[0].y - metaLineCur.points[numCur - 1].y) *
                        (metaLineCur.points[0].y - metaLineCur.points[numCur - 1].y) <
                    0) {
                    // merge
                    for (i = 0; i < numHyp; ++i) {
            ...
            if (dis_s < dis_e && dis_s < thDis) {
                if ((metaLines[metaIDHyp].points[numHyp - 1].x - metaLineCur.points[numCur - 1].x) *
                        (metaLineCur.points[0].x - metaLineCur.points[numCur - 1].x) <
                    0) {
                    // merge
                    for (i = 0; i < numHyp; ++i) {
                        pixel_t& p = metaLineCur.points.push_back();
            ...
            if (dis_e < dis_s && dis_e < thDis) {
                if ((metaLines[metaIDHyp].points[0].x - metaLineCur.points[numCur - 1].x) *
                        (metaLineCur.points[0].x - metaLineCur.points[numCur - 1].x) <
                    0) {
                    // merge
                    for (i = 0; i < numHyp; ++i) {
...
int MetaLine::lineMerging2(int IDCur,
                           line_t& metaLineCur,
                           size_list_t& lineHyps,
                           lines_list_t& metaLines,
                           size_list_t& IDMerged) {
    int i, j;
    float thDev = 2.0;
    float* ptrMI = (float*)maskImage.data;
    ...
    for (i = 0; i < lineHyps.size(); ++i) {
        ...
        deviation2 /= length;
        if (deviation1 + deviation2 > 2 * thDev)
            removal[i] = 1;
    }
            ...
            if (NFA > 1) {
                ...
                if (disMin == dis1) {
                    ...
                    for (j = length - 1; j >= 0; --j) {
                        mergedPoints.push_back(Point2f(metaLineCur.points[j].x, metaLineCur.points[j].y));
                    }
                    for (j = 0; j < lengthCur; ++j) {
                        mergedPoints.push_back(
                            Point2f(metaLines[IDHypsNew[i]].points[j].x, metaLines[IDHypsNew[i]].points[j].y));
                    }
                } else if (disMin == dis2) {
                    pts = Point2f(xs, ys);
...
void MetaLine::subDivision(clusters_list_t& clusters,
                           const string_t& string,
                           const size_t first_index,
                           const size_t last_index,
                           const float min_deviation,
    ...
    for (size_t i = first_index, count = string.size(); i != last_index; i = (i + 1) % count) {
        ...
        deviation = static_cast<float>(
            abs(((current.x - first.x) * (first.y - last.y)) + ((current.y - first.y) * (last.x - first.x))));
    ...
    if ((max_deviation >= min_deviation) && ((max_pixel_index - first_index + 1) >= half_min_size) &&

...
def line_area_intersection(l_src, l_tar):
    """
    This function computes the intersection area between lines in the prediction and the target

    Parameters
    ----------
    l_src : numpy.ndarray
        The lines in the prediction with shape (N, 4) where N is the number of lines
    l_tar : numpy.ndarray
        The lines in the target with shape (N, 4) where N is the number of lines

    Returns
    -------
    numpy.ndarray
        The intersection area between lines in the prediction and the target
    """
    ...
    for i1 in range(nb_line_tar - 1):
        ...
        for i2 in range(i1 + 1, nb_line_tar):
            ...
            elif ts2 >= ts1 and tf2 >= tf1 and ts2 <= tf1:
                # tar1: *----------*
                # tar2:     *--------*
                vec_tar[i2, 0] = tf1
                # tar2:            *-*

            elif ts1 <= ts2 and tf2 <= tf1:
                # tar1: *----------*
                # tar2:     *----*
                vec_tar[i2, :] = 0
                # tar2:
